...
value_bmi <- fread('/mnt/ml4cvd/projects/skhurshid/accel_phewas/cox_data_mvpa_cutoff_bmi.csv')
...
# If less than 10 cases, abort
  if (n_events < 10){
    ...
    result <- data.table(disease,n_events,fu_median,fu_q1,fu_q3,hr,lower,upper,p)
    ...
    n <- n+1; next}
# Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ who_acc_rate_bouted + age_accel + sex + bmi, data=analysis_set)
  hr <- summary(model)$coefficients[5]; lower <- summary(model)$conf.int[9]
  upper <- summary(model)$conf.int[13]; p <- summary(model)$coefficients[17]
  result <- data.table(disease,n_events,fu_median,fu_q1,fu_q3,hr,lower,upper,p)
  out <- rbind(out,result)
  if (n %% 50 == 0){print(paste0("Just finished model ",n," out of ",length(list),"!"))}
  n <- n+1
...
# Save out
write.csv(out,file='/mnt/ml4cvd/projects/skhurshid/accel_phewas/phecode_processed/cox_mvpa_cutoff_bmi.csv',row.names=F)