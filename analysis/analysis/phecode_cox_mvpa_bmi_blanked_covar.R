...
mvpa_bmi <- fread('/mnt/ml4cvd/projects/skhurshid/accel_phewas/cox_data_mvpa_rate_bmi_blanked_covar.csv')
setkey(mvpa_bmi,sample_id)

# Format dates
for (j in (c('blanked_date','phenotype_censor_date'))){set(mvpa_bmi,j=j,value=as.Date(mvpa_bmi[[j]],format='%Y-%m-%d'))}
...
# Create analysis variables
  analysis_set[,time_to_event := ifelse(c(has_disease == 0 | is.na(has_disease)),pmin(as.numeric(censor_date - blanked_date)/365.25,as.numeric(phenotype_censor_date - blanked_date)/365.25),
                                        as.numeric(censor_date - blanked_date)/365.25)]
...
# If less than 10 cases, abort
  if (n_events < 10){
    ...
    n <- n+1; next}
# Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ mvpa_std + age_accel + sex + bmi + sbp + dbp + bpmed + tob + tdi + etoh_grams, data=analysis_set)
...
# Save out
write.csv(out,file='/mnt/ml4cvd/projects/skhurshid/accel_phewas/phecode_processed/cox_mvpa_bmi_blanked_covar.csv',row.names=F)