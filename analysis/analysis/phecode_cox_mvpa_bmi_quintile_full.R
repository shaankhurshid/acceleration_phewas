...
mvpa_bmi <- fread('/mnt/ml4cvd/projects/skhurshid/accel_phewas/cox_data_mvpa_rate_bmi_quintile_covar.csv')
...
# If less than 10 cases, abort
  if (n_events < 10){
    ...
    n <- n+1; next}
# Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ factor(mvpa_decile) + age_accel + sex + bmi + sbp + dbp + bpmed + tob + tdi + etoh_grams + diet + qual_ea, data=analysis_set)
...
# Save out
write.csv(out,file='/mnt/ml4cvd/projects/skhurshid/accel_phewas/phecode_processed/cox_mvpa_bmi_quintile_full.csv',row.names=F)