...
prs <- fread('/mnt/ml4cvd/projects/skhurshid/accel_phewas/cox_data_prs_white.csv')
...
# If less than 10 cases, abort
  if (n_events < 10){
    ...
    n <- n+1; next}
# Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ prs_std + enroll_age + sex 
...
# Save out
write.csv(out,file='/mnt/ml4cvd/projects/skhurshid/accel_phewas/phecode_processed/cox_prs_eur.csv',row.names=F)