...
# If less than 10 cases, abort
  if (n_events < 10){
    ...
    n <- n+1; next}
# Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ self_mvpa + enroll_age + sex + bmi, data=analysis_set)
...
# Save out
write.csv(out,file='/mnt/ml4cvd/projects/skhurshid/accel_phewas/phecode_processed/cox_self_mvpa_bmi.csv',row.names=F)