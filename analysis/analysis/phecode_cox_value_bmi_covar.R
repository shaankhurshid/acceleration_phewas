...
value_bmi <- fread('/mnt/ml4cvd/projects/skhurshid/accel_phewas/cox_data_value_bmi_covar.csv')
...
# If less than 10 cases, abort
  if (n_events < 10){
    beta <- NA; se <- NA; hr <- NA; lower <- NA; upper <- NA; p <- NA; z <- NA
    ...
    n <- n+1; next}
# Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ value_std + age_accel + sex + bmi + sbp + dbp + bpmed + tob + tdi + etoh_grams + diet + qual_ea, data=analysis_set)
...
# Save out
write.csv(out,file='/mnt/ml4cvd/projects/skhurshid/accel_phewas/phecode_processed/cox_value_bmi_covar.csv',row.names=F)