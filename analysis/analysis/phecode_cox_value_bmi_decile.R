...
value_bmi <- fread('/mnt/ml4cvd/projects/skhurshid/accel_phewas/cox_data_value_bmi_decile.csv')
...
# If less than 10 cases, abort
  if (n_events < 10){
    ...
    n <- n+1; next}
# Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ value_decile + age_accel + sex + bmi, data=analysis_set)
...
# Save out
write.csv(out,file='/mnt/ml4cvd/projects/skhurshid/accel_phewas/phecode_processed/cox_value_bmi_decile.csv',row.names=F)