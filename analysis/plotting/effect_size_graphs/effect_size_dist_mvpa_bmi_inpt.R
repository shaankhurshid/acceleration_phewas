...
mvpa_bmi <- fread('~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_mvpa_bmi_processed_fdr_covar_inpt.csv')
...
## ALL
# Plot
pdf('~/Documents/MGH Research/accel_phewas/phecode_plots/effect_size_box_mvpa_bmi_covar_inpt.pdf',pointsize=6,
    height=6,width=9)
par(oma=c(1,1,1,1))
...
boxplot(formula = mvpa_bmi$hr ~ mvpa_bmi$group_median_hr,log='y',
        ...
        cex=1.4,outline=FALSE,ylim=c(0.4,1.4),xlim=c(0,length(unique(mvpa_bmi$category))),
        at=1:length(unique(mvpa_bmi$category)))

# Axes
axis(1,cex.axis=2,at=1:length(unique(mvpa_bmi$category)),labels = NA)
axis(2,cex.axis=2,las=2,pos=0,at=seq(0.4,1.4,0.2))
