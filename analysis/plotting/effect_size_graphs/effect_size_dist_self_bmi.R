...
value_bmi <- fread('~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_self_bmi_processed_fdr_covar.csv')
...
## ALL
# Plot
pdf('~/Documents/MGH Research/accel_phewas/phecode_plots/effect_size_box_self_bmi_covar.pdf',pointsize=6,
    height=6,width=9)
par(oma=c(1,1,1,1))
par(mar=c(15,4,2,8.5))

# Plot
boxplot(formula = value_bmi$hr ~ value_bmi$group_median_hr,log='y',
        col=unique(value_bmi[order(value_bmi$group_median_hr)]$col),xlab='',ylab='',xaxt='n',yaxt='n',frame=F,
        cex=1.4,outline=FALSE,ylim=c(0.4,1.4),xlim=c(0,length(unique(value_bmi$category))),
