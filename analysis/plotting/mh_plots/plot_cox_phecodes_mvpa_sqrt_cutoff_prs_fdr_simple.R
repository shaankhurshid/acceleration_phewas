...
results <- fread(file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_mvpa_sqrt_cutoff_prs.csv')
...
# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_mvpa_sqrt_cutoff_prs_fdr_simple.pdf',height=5,width=12,pointsize=5)
...
plot(x=1:nrow(results),y=-log10(results$p),col=ifelse(!is.na(results$sig),results$col,paste0(results$col,'4D')),
     ...
     ylim=c(0,15),xlab='',ylab='',cex=2,pch=results$shape)

axis(1,at=x_locs$x_coord,cex.axis=2,labels=rep('',length(x_locs$x_coord)))
axis(2,cex.axis=2,at=seq(0,15,5),las=2,pos=-12)
...
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_mvpa_sqrt_cutoff_prs_effect_fdr.pdf',height=13,width=20,pointsize=5)
par(mar=c(33,3,1,12),oma=c(1,1,1,1))

plot(x=1:nrow(sig_results),y=sig_results$hr,col=sig_results$col,
     bty='n',xaxt='n',yaxt='n',pch=19,xlim=c(0,nrow(sig_results)),
     ylim=c(0.4,1.8),xlab='',ylab='',cex=3.5)

axis(1,at=1:nrow(sig_results),cex.axis=2.2,labels=rep('',nrow(sig_results)))
axis(2,cex.axis=3.2,at=seq(0.4,1.8,0.2),las=2,pos=0.5)

mtext("Hazard ratio",2,line=0.5,cex=3.2)

segments(0.6,1,nrow(sig_results),1,col='#bd0026',lty=2)

segments(1:nrow(sig_results),sig_results$lower,1:nrow(sig_results),pmin(sig_results$upper,1.8),col=sig_results$col,
         lwd=2.2)

# Truncated CIs
arrows(x0=nrow(sig_results) - 1,y0=sig_results$lower[length(sig_results$lower) - 1],
       x1=nrow(sig_results) - 1,y1=1.8,
       angle=45,length=0.1,lwd=1.5,code=2,col=sig_results$col[length(sig_results$col) - 1])

arrows(x0=nrow(sig_results),y0=sig_results$lower[length(sig_results$lower)],
       x1=nrow(sig_results),y1=1.8,
       angle=45,length=0.1,lwd=1.5,code=2,col=sig_results$col[length(sig_results$col)])
...
write.csv(results,'~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_mvpa_sqrt_cutoff_prs_processed_fdr.csv')
