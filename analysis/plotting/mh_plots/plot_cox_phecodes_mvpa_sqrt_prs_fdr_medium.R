...
results <- fread(file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_mvpa_sqrt_prs_medium.csv')
...
# Remove non-sensical betas (did not converge)
...
# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_mvpa_sqrt_prs_fdr_medium.pdf',height=5,width=12,pointsize=5)
par(mar=c(15,4,1,5),oma=c(1,1,1,1))
...
plot(x=1:nrow(results),y=-log10(results$p),col=ifelse(!is.na(results$sig),results$col,paste0(results$col,'4D')),
     ...
     ylim=c(0,15),xlab='',ylab='',cex=2.2,pch=results$shape)

axis(1,at=x_locs$x_coord,cex.axis=2.4,labels=rep('',length(x_locs$x_coord)))
axis(2,cex.axis=2.4,at=seq(0,15,5),las=2,pos=-12)
...
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_mvpa_sqrt_prs_effect_fdr_medium.pdf',height=7,width=16,pointsize=5)
...
plot(x=1:nrow(sig_results),y=sig_results$hr,col=sig_results$col,
     ...
     ylim=c(0.7,1.3),xlab='',ylab='',cex=2.5)

axis(1,at=1:nrow(sig_results),cex.axis=2,labels=rep('',nrow(sig_results)))
axis(2,cex.axis=2.2,at=seq(0.7,1.3,0.1),las=2,pos=0.5)
...
write.csv(results,'~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_mvpa_sqrt_prs_processed_fdr_medium.csv')
