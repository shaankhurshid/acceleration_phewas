...
results <- fread(file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_self_mvpa_cutoff_bmi.csv')
...
# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_self_mvpa_cutoff_fdr_simple.pdf',height=5,width=12,pointsize=5)
...
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_self_mvpa_cutoff_effect_fdr.pdf',height=13,width=16,pointsize=5)
par(mar=c(32,3,1,18),oma=c(1,1,0,1))

plot(x=1:nrow(sig_results),y=sig_results$hr,col=sig_results$col,
     bty='n',xaxt='n',yaxt='n',pch=19,xlim=c(0,nrow(sig_results)),
     ylim=c(0.4,2.2),xlab='',ylab='',cex=2.8)

axis(1,at=1:nrow(sig_results),cex.axis=2,labels=rep('',nrow(sig_results)))
axis(2,cex.axis=2.8,at=seq(0.4,2.2,0.4),las=2,pos=0.5)
...
write.csv(results,'~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_self_mvpa_cutoff_processed_fdr.csv')
