...
# Load PRS
prs_set <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/prs/scores/mvpa_sqrt_prs4/mvpa_sqrt_prs4_processed.csv')
prs_in_analysis <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/cox_data_mvpa_sqrt_prs.csv')

# Load accel genetic file
acceleration <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/mvpa_rate_bouted_sqrt.tsv')

# Join
setkey(prs_set,sample_id); setkey(acceleration,FID)
acceleration[prs_set,':='(prs_std = i.prs_std, sum_score = i.sum_score)]

# Remove no PRS (poor quality genetic data)
prs_set <- acceleration[!is.na(prs_std)]

# Check exposure ~ PRS
mod <- lm(mvpa_rate_bouted_sqrt ~ sum_score,data=prs_set)

# Now fit genetic acceleration model
mod_fitted <- lm(mvpa_rate_bouted_sqrt ~ sum_score + PC1 + PC2 + PC3 + PC4 + PC5 + array_UKBB + male + age_accel,data=prs_set)

# Now load the no accel sets
prs_na <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/prs/scores/mvpa_sqrt_prs_no_accel/mvpa_sqrt_prs_no_accel_processed.csv')
...
prs_no_accel <- prs_no_accel[,c('sample_id','sex','enroll_age','array_UKBB','prs_std','sum_score',paste0('PC',1:5))]
...
# Remove small number of people who fell out of analysis
prs_no_accel <- prs_no_accel[sample_id %in% prs_in_analysis$sample_id]
...
write.csv(prs_no_accel,file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/no_accel_mvpa_sqrt_prs_phenotype.csv',row.names=F)
