...
for (j in c('d2','d3','d4','d5','d6','d7','d8','d9','d10')){
...
# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_mvpa_bmi_decile_full.pdf',height=5,width=12,pointsize=5)
par(mar=c(15.6,10,1,5),oma=c(1,1,1,1))

y_vals <- c(results$d2,results$d3,results$d4,results$d5,results$d6,results$d7,results$d8,results$d9,results$d10)
col_vals <- c(rep('#d730278C',355),rep('#f46d438C',355),rep('#fdae618C',355),rep('#fee08b8C',355),
              rep('#ffffbf8C',355),rep('#d9ef8b8C',355),rep('#a6d96a8C',355),rep('#66bd638C',355),
              rep('#1a98508C',355))
x_vals <- c(int1,int2,int3,int4,int5,int6,int7,int8,int9,int10,
            int11,int12,int13,int14,int15,int16,int17)

plot(x=rep(x_vals,9),y=y_vals,col=col_vals,
     bty='n',xaxt='n',yaxt='n',xlim=c(0,max(int17)),
     ylim=c(0,2),xlab='',ylab='',cex=2.2,pch=19)

axis(1,at=x_locs$x_coord_new,cex.axis=2.4,labels=rep('',length(x_locs$x_coord_new)))
axis(2,cex.axis=2.4,at=c(0,0.1,0.5,1,1.5,2),las=2,pos=-12)

segments(0,1,max(int17),1,col='black',lty=5)

mtext("Hazard ratio versus lowest MVPA decile",2,line=5,cex=2.4)
