...
results <- fread(file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_vpa_bmi_quintile_full.csv')
...
primary_results <- fread(file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_vpa_bmi_processed_fdr_covar.csv')
...
x_locs$x_coord_new <- c(mean(int1),mean(int2),mean(int3),mean(int4),mean(int5),mean(int6),mean(int7),
                        ...
                        mean(int15),mean(int16))

# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_vpa_bmi_quintile_full.pdf',height=5,width=12,pointsize=5)
par(mar=c(15.6,10,1,10),oma=c(1,1,1,1))

y_vals <- c(results$d2,results$d3,results$d4,results$d5)
col_vals <- c(rep('#d7191c8C',122),rep('#fdae618C',122),rep('#a6d96a8C',122),rep('#1a96418C',122))
...
plot(x=rep(x_vals,4),y=y_vals,col=col_vals,bg=col_vals,
     bty='n',xaxt='n',yaxt='n',xlim=c(0,max(int16)),
     ylim=c(0,1.5),xlab='',ylab='',cex=2.2,pch=19)

axis(1,at=x_locs$x_coord_new,cex.axis=2.4,labels=rep('',length(x_locs$x_coord_new)))
axis(2,cex.axis=2.4,at=c(0,0.5,1,1.5),las=2,pos=-12)

segments(0,1,max(int16),1,col='black',lty=5)
...
par(xpd=TRUE)
legend(x=max(int16)+10,y=1.3,legend=c('','','',''),pch=19,cex=2.4,y.intersp = 2,
       col=c('#d7191c8C','#fdae618C','#a6d96a8C','#1a96418C'),bty='n')
dev.off()

write.csv(results,file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_vpa_bmi_quintile_full_covar_processed.csv',row.names=F)