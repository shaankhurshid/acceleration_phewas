...
results <- fread(file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_mvpa_bmi_quintile_covar.csv')
...
# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_mvpa_bmi_min_quintile.pdf',height=5,width=12.5,pointsize=5)
...
plot(x=1:nrow(results),y=results$min_decile,col=results$col,
     ...
     ylim=c(0.5,5.5),xlab='',ylab='',cex=2.2,pch=19)

axis(1,at=x_locs$x_coord,cex.axis=2.4,labels=rep('',length(x_locs$x_coord)))
axis(2,cex.axis=2.4,at=seq(1,5,1),las=2,pos=-1,labels=rep('',5))

mvpa_labels <- c('<45','45-99','100-175','176-284','>284')

mtext("Weekly MVPA With Lowest Risk (min)",2,line=8,cex=2.4)

text(x = par("usr")[1] + 6.5,
     y = 1:5,
     labels = mvpa_labels,
     xpd = NA,
     srt = 30,
...
# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_mvpa_bmi_max_quintile.pdf',height=5,width=12.5,pointsize=5)
par(mar=c(15.6,10,1,2),oma=c(1,1,1,1))
...
mtext("Weekly MVPA With Highest Risk (min)",2,line=8,cex=2.4)
...
write.csv(results,file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_mvpa_bmi_quintile_covar_processed.csv',row.names=F)