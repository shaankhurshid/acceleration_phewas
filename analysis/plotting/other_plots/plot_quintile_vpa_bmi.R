...
results <- fread(file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_vpa_bmi_quintile_covar.csv')
...
# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_vpa_bmi_min_quintile.pdf',height=5,width=12.5,pointsize=5)
...
mvpa_labels <- c('<37','37-76','77-138','139-278','>278')

mtext("Weekly VPA With Lowest Risk (min)",2,line=8,cex=2.4)

text(x = par("usr")[1] + 2.5,
     y = 1:5,
     labels = mvpa_labels,
     xpd = NA,
     srt = 30,
...
# Plot p-values
pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_vpa_bmi_max_quintile.pdf',height=5,width=12.5,pointsize=5)
par(mar=c(15.6,10,1,2),oma=c(1,1,1,1))
...
write.csv(results,file='~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_vpa_bmi_quintile_covar_processed.csv',row.names=F)