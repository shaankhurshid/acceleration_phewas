...
# Load data
mvpa_bmi <- fread('~/Documents/MGH Research/accel_phewas/phecode_outputs/cox_mvpa_sqrt_prs_processed_fdr.csv')

# Top 3 conditions per category that are significant
setkey(mvpa_bmi,hr)
...
results[phenotype=='Symptoms involving skin and other integumentary tissue','phenotype'] <- "Symptoms involving skin"

pdf(file='~/Documents/MGH Research/accel_phewas/phecode_plots/cox_mvpa_prs_effect_zoomed.pdf',height=9,width=12,pointsize=5)
par(mar=c(30,4,1,12),oma=c(1,1,1,1),xpd=TRUE)

plot(x=x_coord,y=results$hr,col=results$col,
     bty='n',xaxt='n',yaxt='n',pch=19,xlim=c(0,max(x_coord)),
     ylim=c(0.25,1),xlab='',ylab='',cex=2.5)

axis(1,at=x_coord,cex.axis=2,labels=rep('',nrow(results)))
axis(2,cex.axis=2.2,at=seq(0.25,1,0.25),las=2,pos=0)
...
legend(x=30,y=0.36,legend=unique(results$category),col=unique(results$col),ncol=4,
