...
hits_cutoff <- cutoff_bmi[sig==1 & hr < 1,.N,by='category']; total_cutoff <- cutoff_bmi[,.N,by='category']
hits_self <- self_bmi[sig==1 & hr < 1,.N,by='category']; total_self <- self_bmi[,.N,by='category']
hits_prs <- prs[sig==1 & hr < 1,.N,by='category']; total_prs <- prs[,.N,by='category']

setkey(hits_cutoff,category); setkey(total_cutoff,category)
setkey(hits_self,category); setkey(total_self,category); setkey(hits_prs,category); setkey(total_prs,category)

hits_cutoff[total_cutoff,total := i.N]
hits_self[total_self,total := i.N]
hits_prs[total_prs,total := i.N]

hits_cutoff[,fraction := N/total]
hits_self[,fraction := N/total]
hits_prs[,fraction := N/total]

setorder(hits_cutoff,-'fraction'); setorder(hits_self,-'fraction'); setorder(hits_prs,-'fraction')
...
col_corr[hits_prs,':='(e_hits_prs_fraction = i.fraction)]
...
melted <- melt(col_corr,id.vars=c('all_categories','cat_col','category_ordered'),
               measure.vars = c('c_hits_cutoff_fraction',
                                'd_hits_self_fraction','e_hits_prs_fraction'))
...
coords <- barplot(melted$value,space=c(rep(0.05,3),1,rep(c(rep(0.05,2),1),16),rep(0.05,2)),border=NA,
                  col=rep(c('#fdb462','#b3de69','#fccde5'),18),ylim=c(0,0.6),
                  yaxt='n',xaxt='n')

axis(2,cex.axis=2.5,las=2,at=seq(0,0.6,0.1),pos=-1,labels=as.character(0:6))
mtext("% of Category Associated (Lower Risk)",2,cex=3,line=2.5)
...
setkey(hits_prs,category); setkey(total_prs,category)
...
coords <- barplot(melted$value,space=c(rep(0.05,3),1,rep(c(rep(0.05,2),1),16),rep(0.05,2)),border=NA,
                  col=rep(c('#fdb462','#b3de69','#fccde5'),18),ylim=c(0.3,0),
                  yaxt='n',xaxt='n')

axis(2,cex.axis=2.5,las=2,at=seq(0.3,0,-0.1),pos=-1,labels=as.character(3:0))
mtext("% of Category Associated (Higher Risk)",2,cex=3,line=2.5)
