...
# Load PRS
prs <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/prs/scores/cutoff_prs4/cutoff_prs4_processed.csv')

# Load accel genetic file
acceleration <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/guideline.tsv')
...
# Check exposure ~ PRS
mod <- lrm(guideline ~ prs_std,data=prs_set)

# Now fit genetic acceleration model
mod_fitted <- glm(guideline ~ prs_std + PC1 + PC2 + PC3 + PC4 + PC5 + array_UKBB + male + age_accel,family='binomial',data=prs_set)
...
write.csv(prs_no_accel,file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/no_accel_cutoff_prs_phenotype.csv',row.names=F)
