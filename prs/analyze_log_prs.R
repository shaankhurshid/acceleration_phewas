...
# Load PRS
prs <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/prs/scores/mvpa_log_prs4/mvpa_log_prs4_processed.csv')

# Load accel genetic file
acceleration <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/mvpa_rate_bouted_log.tsv')
...
# Check exposure ~ PRS
mod <- lm(mvpa_rate_bouted_log ~ prs_std,data=prs_set)

# Now fit genetic acceleration model
mod_fitted <- lm(mvpa_rate_bouted_log ~ prs_std + PC1 + PC2 + PC3 + PC4 + PC5 + array_UKBB + male + age_accel,data=prs_set)

# Now load the no accel sets
prs_na <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/prs/scores/prs_no_accel/prs_no_accel_processed.csv')
...
# Actual vs Inferred
x <- list(v1=exp(prs_set$mvpa_rate_bouted_log),v2=exp(prs_set$inferred_accel))
data <- melt(x)
...
ggsave('~/Documents/MGH Research/accel_phewas/accel_mvpa_log_density_compare.pdf',
       height=2,width=2.5,units='in',scale=4)

# Figure out expected counts for guidelines
prs_set[,':='(guideline_inferred = ifelse(exp(inferred_accel) >= 150/7,1,0))]
