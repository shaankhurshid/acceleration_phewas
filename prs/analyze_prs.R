...
# Load PRS
prs <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/prs/scores/prs4/prs4_processed.csv')
...
acceleration <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/value.tsv')
...
# Check exposure ~ PRS
mod <- lm(value ~ prs_std,data=prs_set)

# Now fit genetic acceleration model
mod_fitted <- lm(value ~ prs_std + PC1 + PC2 + PC3 + PC4 + PC5 + array_UKBB + male + age_accel,data=prs_set)
