...
# Load pre-seed data
prs <- fread(file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/no_accel_mvpa_sqrt_sensi_prs_phenotype.csv')

# Merges
setkey(prs,sample_id); setkey(censor_data,sample_id)
censor_data[prs,':='(inferred_accel = i.guideline_inferred, PC1 = i.PC1, PC2 = i.PC2, PC3 = i.PC3, PC4 = i.PC4, PC5 = i.PC5)]
...
prs <- prs[,c('sample_id','enroll_age','enroll_date','sex','inferred_accel','phenotype_censor_date',paste0('PC',1:5))]

# Write out
write.csv(prs,file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/cox_data_mvpa_sqrt_sensi_cutoff_prs.csv',row.names = F)
