...
prs[,inferred_accel_invnorm := qnorm((rank(inferred_accel,na.last="keep")-0.5)/sum(!is.na(inferred_accel)))]

# Scope columns for PRS analysis
prs <- prs[,c('sample_id','enroll_age','enroll_date','sex','inferred_accel_invnorm','phenotype_censor_date',paste0('PC',1:5))]

# Write out
write.csv(prs,file='/Volumes/medpop_afib/skhurshid/acceleration_phewas/cox_data_prs_invnorm.csv',row.names = F)
