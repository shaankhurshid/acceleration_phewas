...
# Load self data
self <- fread(file='self_phenotype.csv')

### BMI
bmi <- fread(file='bmi_instance0_202109.csv')

## Smoking
tob <- fread('tobacco_instance_0.csv')
tob[,tobacco_accel_selfreport := ifelse(value==2,"Current",
                                        ifelse(value==1,"Former","Never"))]

## TDI
tdi <- fread(file='townsend_0.csv')

## EtOH
etoh <- fread(file='etoh_0_012022.csv')

## BP
sbp <- fread('sbp_combined_instance0_202006.csv')
dbp <- fread('dbp_combined_instance0_202006.csv')

## anti-HTN
bpmed <- fread('bpmed_combined_instance0.csv')

# Load withdrawals
withdrawals <- fread(file='w7089_20210809.csv')
...
# Remove withdrawals
censor_data <- censor_data[!(sample_id %in% withdrawals$V1)] #456443 - 23 = 456420
...
# Write out raw
write.csv(self,file='cox_data_self_bmi_raw_covar.csv',row.names = F)

# Standardize self MVPA
self[,':='(self_mvpa = (self_mvpa - mean(self_mvpa,na.rm=T))/sd(self_mvpa,na.rm=T))]

# Write out
write.csv(self,file='cox_data_self_bmi_covar.csv',row.names = F)
